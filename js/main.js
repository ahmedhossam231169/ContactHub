...
function addContact() {
  ...
  // Prevent duplicate phone numbers across contacts (normalize before comparing)
  for (var i = 0; i < total.length; i++) {
    if (editingIndex !== null && i === editingIndex) continue;
    var existingNorm = getNormalizedPhone(total[i].phone);
    if (existingNorm && existingNorm === digits) {
      showValidation('This phone number is already used by another contact.', contactPhone);
      return;
    }
  }
